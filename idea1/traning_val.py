...
def train_fn(loader_train1,loader_valid1,model1, optimizer1, scaler):  ### Loader_1--> ED and Loader2-->ES
    ...
    for batch_idx, (img_LA_ES,temp_LA_ES,img_SA_ES,temp_SA_ES,img_LA_ED,temp_LA_ED,img_SA_ED,temp_SA_ED,label,M,all_three_ES,all_three_ED,three_ES_SA,three_ED_SA) in enumerate(loop):
       ...
       with torch.cuda.amp.autocast():
             ...
             encoded_2d,encoded_3d,x_2,x_3,x_2_projected,x_3_projected = model1(temp_LA_ES,temp_SA_ES)
             
             #print(encoded_2d.shape)
             #print(temp_LA_ES.shape)
             loss1 = loss_fn_mse(x_2_projected, x_3)
             loss2 = loss_fn_mse(x_3_projected, x_2)
             loss3 = loss_fn_dice(encoded_2d, temp_LA_ES)
             loss4 = loss_fn_dice(encoded_3d, temp_SA_ES)
             loss_1 = (loss1+loss2+loss3+loss4)/4
             
             encoded_2d,encoded_3d,x_2,x_3,x_2_projected,x_3_projected = model1(temp_LA_ED,temp_SA_ED)
             loss1 = loss_fn_mse(x_2_projected, x_3)
             loss2 = loss_fn_mse(x_3_projected, x_2)
             loss3 = loss_fn_dice(encoded_2d, temp_LA_ED)
             loss4 = loss_fn_dice(encoded_3d, temp_SA_ED)
...
def check_Acc(loader, model1, device=DEVICE):
    loop = tqdm(loader)
...
path_to_save_Learning_Curve='/data/scratch/acw676/VAE_weights/'+'/UNet_1'
path_to_save_check_points='//data/scratch/acw676/VAE_weights/'+'/UNet_1'  ##these are weights
### 3 - this function will save the check-points 
def save_checkpoint(state, filename=path_to_save_check_points+".pth.tar"):
    print("=> Saving checkpoint")
    torch.save(state, filename)
 
from proj_models import Recons_Model
model_1 = Recons_Model()
...
def main():
    ...
    for epoch in range(Max_Epochs):
        ...
        print(print_msg)
        dice_score = check_Acc(val_loader, model1, device=DEVICE)
