...
Base = 64
class Model_3D_to_2D(nn.Module):
    def __init__(self, n_channels = 1, bilinear=False):
        super(Model_3D_to_2D, self).__init__()
        self.n_channels = n_channels
        
        self.encoder_3d = nn.Sequential( 
        Conv_3d(n_channels, Base,1,3),
        Conv_3d(Base, Base,(1,2,2),3),
        Conv_3d(Base, 2*Base,1,3),
        Conv_3d(2*Base, 2*Base,(1,2,2),3),
        Conv_3d(2*Base,4*Base,1,3), 
        Conv_3d(4*Base, 4*Base,(1,2,2),3), 
        Conv_3d(4*Base,4*Base,1,3), 
        Conv_3d(4*Base, Base,(1,2,2),3), 
        )
        self.encoder_2d = nn.Sequential( 
        Conv_2d(n_channels, Base,1,3),
        Conv_2d(Base, Base,1,3),
        Conv_2d(Base, 2*Base,1,3),
        Conv_2d(2*Base, 2*Base,1,3),
        Conv_2d(2*Base,4*Base,1,3), 
        Conv_2d(4*Base, 4*Base,1,3), 
        Conv_2d(4*Base,4*Base,1,3), 
        Conv_2d(4*Base, Base,1,3), 
        )
        
        
        self.decoder_2d =  nn.Sequential(
        Deconv_2d(Base,4*Base,2,2),
        Conv_2d(4*Base,4*Base,1,3),
        ...
        Deconv_3d(Base,4*Base,2,2),
        Conv_3d(4*Base,4*Base,1,3),
        Deconv_3d(4*Base,2*Base,2,2),
        Conv_3d(2*Base,2*Base,1,3),
        Deconv_3d(2*Base,Base,2,2),
        Conv_3d(Base,Base,1,3),
        Deconv_3d(Base,Base,2,2),
        Conv_3d(Base,Base,1,3),
        OutConv_2d(Base,1)
        )
    ...
    def forward(self, x_2d,x_3d):
      ...
      out_2d = self.decoder_2d(emb_2d)
      return out_2d
  
Input_Image_Channels = 1
def model() -> Model_3D_to_2D:
    model = Model_3D_to_2D()
    return model
