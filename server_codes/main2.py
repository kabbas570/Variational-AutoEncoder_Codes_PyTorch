...
def train_fn(loader_train1,loader_valid1,model1,model2, optimizer1, scaler,loss_fn):  ### Loader_1--> ED and Loader2-->ES
    ...
    for batch_idx, (img_LA_ES,temp_LA_ES,img_SA_ES,temp_SA_ES,img_LA_ED,temp_LA_ED,img_SA_ED,temp_SA_ED,label,M,all_three_ES,all_three_ED,three_ES_SA,three_ED_SA) in enumerate(loop):
       ...
       with torch.cuda.amp.autocast():
             ...
             seg = model1(img_LA_ES)
             recon_batch = model2.encoder(all_three_ES)
             loss1 = loss_fn1(seg, recon_batch)
             
             seg = model1(img_LA_ED)
             recon_batch = model2.encoder(all_three_ED)
             loss2 = loss_fn1(seg, recon_batch)
...
def check_Acc(loader, model1,model2,loss_fn, device=DEVICE):
    loop = tqdm(loader)
    Three_ES = 0
    Three_ED = 0
    ...
    for batch_idx, (img_LA_ES,temp_LA_ES,img_SA_ES,temp_SA_ES,img_LA_ED,temp_LA_ED,img_SA_ED,temp_SA_ED,label,M,all_three_ES,all_three_ED,three_ES_SA,three_ED_SA) in enumerate(loop):
       ...
       with torch.cuda.amp.autocast():
            ...
            seg = model1(img_LA_ES)
            pred_ES = model2.decoder(seg)

            pred_ES = (pred_ES > 0.5)*1
            single_ES = (2 * (pred_ES * all_three_ES).sum()) / (
                (pred_ES + all_three_ES).sum() + 1e-8)
            
            Three_ES +=single_ES

            seg = model1(img_LA_ED)
            pred_ED = model2.decoder(seg)
                        
            
            pred_ED = (pred_ED > 0.5)*1
            single_ED = (2 * (pred_ED * all_three_ED).sum()) / (
                (pred_ED + all_three_ED).sum() + 1e-8)
...
path_to_save_Learning_Curve='/data/scratch/acw676/VAE_weights/'+'/UNet'
path_to_save_check_points='//data/scratch/acw676/VAE_weights/'+'/UNet'  ##these are weights
### 3 - this function will save the check-points 
def save_checkpoint(state, filename=path_to_save_check_points+".pth.tar"):
    print("=> Saving checkpoint")
    torch.save(state, filename)
 
from models import Segmentar, Autoencoder_FC
model_1 = Segmentar()
model_2 = Autoencoder_FC()
...
def main():
    ...
    weights_paths= "/data/scratch/acw676/VAE_weights/Autoencoder_FC_a.pth.tar"
    ...
    for epoch in range(Max_Epochs):
        train_loss,valid_loss=train_fn(train_loader,val_loader, model1,model2, optimizer1,scaler,loss_function)
        ...
        print(print_msg)
        dice_score = check_Acc(val_loader, model1,model2,loss_function, device=DEVICE)
