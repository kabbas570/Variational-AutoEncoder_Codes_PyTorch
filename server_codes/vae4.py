...
print(len(valid_loader))   ### same here
...
def compute_epoch_loss_autoencoder(model, features):
    loss_fn =  F.mse_loss
    model.eval()
...
def train_fn(loader_train1,loader_valid1,model1, optimizer1, scaler,loss_fn):  ### Loader_1--> ED and Loader2-->ES
    ...
    for batch_idx, (imgs, _) in enumerate(loop_v):
        ...
        loss1 = compute_epoch_loss_autoencoder(model1,imgs)
...
path_to_save_Learning_Curve='/data/scratch/acw676/VAE_weights/'+'/VAE_4'
path_to_save_check_points='//data/scratch/acw676/VAE_weights/'+'/VAE_4'  ##these are weights
...
#    optimizer.load_state_dict(checkpoint['optimizer'])
    ...
    for epoch in range(Max_Epochs):
        train_loss,valid_loss=train_fn(train_loader,valid_loader, model1, optimizer1,scaler,loss_function)
        ...
        dice_score = check_Acc(valid_loader, model1,loss_function, device=DEVICE)
