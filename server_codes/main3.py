...
#            recon_batch, z_mean, z_log_var = model1(all_three_ES)
#            recon_batch = act(recon_batch)
#           
#            loss1 = loss_fn(z_mean, z_log_var, recon_batch,all_three_ES)
#            recon_batch, z_mean, z_log_var = model1(all_three_ED)
#            recon_batch = act(recon_batch)
#            
#            loss2 = loss_fn(z_mean, z_log_var, recon_batch,all_three_ED)
            
             recon_batch = model1(all_three_ES)
             loss1 = loss_fn1(recon_batch, three_ES_SA)
             recon_batch = model1(all_three_ED)
             loss2 = loss_fn1(recon_batch,three_ED_SA)
...
#            recon_batch = act(recon_batch)
#            
#            loss1 = loss_fn1(recon_batch,all_three_ES)
#            recon_batch, z_mean, z_log_var = model1(all_three_ED)
#            recon_batch = act(recon_batch)
#            
#            loss2 = loss_fn1(recon_batch,all_three_ED)
...
def check_Acc(loader, model1,loss_fn, device=DEVICE):
    ...
    for batch_idx, (img_LA_ES,temp_LA_ES,img_SA_ES,temp_SA_ES,img_LA_ED,temp_LA_ED,img_SA_ED,temp_SA_ED,label,M,all_three_ES,all_three_ED,three_ES_SA,three_ED_SA) in enumerate(loop):
       ...
       with torch.cuda.amp.autocast():
#            recon_batch_ES, z_mean, z_log_var = model1(all_three_ES)
#            recon_batch_ES = act(recon_batch_ES)
            
            recon_batch_ES = model1(all_three_ES)

            recon_batch_ES = (recon_batch_ES > 0.5)*1
            single_ES = (2 * (recon_batch_ES * three_ES_SA).sum()) / (
                (recon_batch_ES + three_ES_SA).sum() + 1e-8)
            
            Three_ES +=single_ES
                
#            recon_batch_ED, z_mean, z_log_var = model1(all_three_ED)
#            recon_batch_ED = act(recon_batch_ED)

            recon_batch_ED = model1(all_three_ED)
            
            recon_batch_ED = (recon_batch_ED > 0.5)*1
            single_ED = (2 * (recon_batch_ED * three_ED_SA).sum()) / (
                (recon_batch_ED + three_ED_SA).sum() + 1e-8)
    ...
    print(f"Dice Score is   : {Three_Overall/len(loader)}")
    return Three_Overall/len(loader)
    
### 6 - This is Focal Tversky Loss loss function ### 
     
epoch_len = len(str(Max_Epochs))

path_to_save_Learning_Curve='/data/scratch/acw676/VAE_weights/'+'/Autoencoder_FC'
path_to_save_check_points='//data/scratch/acw676/VAE_weights/'+'/Autoencoder_FC'  ##these are weights
### 3 - this function will save the check-points 
def save_checkpoint(state, filename=path_to_save_check_points+".pth.tar"):
    print("=> Saving checkpoint")
    torch.save(state, filename)
 
from reg_model import Autoencoder_FC

model_1 = Autoencoder_FC()
