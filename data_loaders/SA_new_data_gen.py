...
class Dataset_io(Dataset): 
    ...
    def __getitem__(self, index):
        ...
        img_SA_ES = np.expand_dims(img, axis=0)
        ...
        temp_SA_ES = generate_label_3(img) 
        ...
        img = Cropping_3d(org_dim3,org_dim1,org_dim2,DIM_,img)
        img_SA_ED = np.expand_dims(img, axis=0)
        ...
        img = Cropping_3d(org_dim3,org_dim1,org_dim2,DIM_,img) 
        img = np.expand_dims(img, axis=0)
        temp_SA_ED = generate_label_3(img) 
        #temp_SA_ED = same_depth(img)
        
      
        return img_SA_ES,temp_SA_ES,img_SA_ED,temp_SA_ED
...
f1=r'C:\My_Data\M2M Data\save2\GT/'
f2=r'C:\My_Data\M2M Data\save2\IMG/'
...
for i in range(1):
    ...
    for k in range(129):
        ...
        plt.imsave(f1 + str(i)+ '__'+str(k) + '.png',gt[2,k,:])
